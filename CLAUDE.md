...
This repo implements Royco's structuring substrates. Dawn is implemented; Dusk is being added, and the repo will support both kernels side by side. The two substrates share their core machinery: tranches, the per-tranche NAVs, the accountant's PnL waterfall, and the YDM.

The formal spec lives in `spec/`, a git submodule tracking [roycoprotocol/royco-dusk-paper](https://github.com/roycoprotocol/royco-dusk-paper). It specifies Dusk, but its first section also documents the shared tranche/waterfall machinery, so it is the source of truth for both the Dusk design and the accountant properties Dawn relies on. Read the relevant LaTeX sections (prefer them over the compiled PDF) before changing accounting, NAV, valuation, or solver logic:
