...
core machinery: tranches, the per tranche NAVs tracked, and the profit and loss
waterfall.
...
requires distinguishing what each tranche \emph{holds} from what it is
...
\begin{itemize}
  ...
  \item \textbf{Effective NAV:} The value that each tranche \emph{is entitled to} after executing their obligations to one another.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Attribute:} Past coverage and risk premiums can leave a
    ...
    claim on the \emph{other} tranche's holdings. Each side's raw NAV deltas are
    split pro rata over the claims on it. The result is a pair of
    effective NAV deltas that sum to the total profit and loss.
  \item \textbf{Settle:} The effective deltas are routed through the
    ...
    \begin{itemize}
      \item \emph{Loss:} Utilize JT's coverage buffer ($\JTeff$) before ST books
        ...
        loss} (JT's debt to ST). Note that JT IL is a transient debt obligation that only 
        exists during the market's configured fixed-term regime: a period of time that
        allows the tranched asset to recover before the junior tranche realizes any losses.
...
\]
The semicolon separates variables from parameters: supplying the
checkpoint yields a function of the raw NAVs alone, written
$F_{\mathrm{cp}}$. The checkpoint holds the previous sync's raw NAVs,
...
\begin{enumerate}
  \item \textbf{NAV conservation:} the inputs sum to the outputs,
    ...
    itself conserves. All stored checkpoints are guaranteed to since they
    are a prior output of $F$. $F$ only redistributes value between the tranches,
    it never creates or destroys it.
  \item \textbf{Piecewise-affine:} each branch is affine in its inputs,
    with slope $\in [0, 1]$. Combined with NAV conservation, this makes
    $F$ continuous, non-decreasing, and 1-Lipschitz in each input. This
    is the property invoked in the existence proof
    (\cref{sec:existence}).
\end{enumerate}
...
Dawn transforms the \emph{risk profile} of any asset. However, 
it doesn't transform its \emph{liquidity profile}:
senior shares have no exit path other than primary redemption.

Dusk extends the junior tranche to transform both. In Dawn, junior
capital may sit in any priceable asset, Treasuries, money market
funds, coinvested alongside the senior tranche, provided its value is
exogenous to the market itself. Dusk drops that
restriction: junior capital is deployed into an AMM pool paired against the
senior tranche's shares.
...
For the purposes of this document, the Dusk JT's asset is a Balancer V3 Pool Token (BPT)
with the constituents being the senior tranche share and a stable quote asset 
(\code{USDC}, \code{sUSDS}, \code{srRoyUSDC}, etc.).
...
\begin{tikzpicture}[
  ...
  \draw (st) to[bend left=30] node[left=8pt]   {value the senior share} (y);
  \node[gray] at (0,0) {\Large$\circlearrowright$};
\end{tikzpicture}
\caption{The Dusk valuation cycle. Each quantity requires the next, so
a sync must find the rate the cycle reproduces.}
\label{fig:recursion}
\end{figure}
...
share), must be determined simultaneously. Exactly one value of
$\STeff$ satisfies both dependencies at once: price the BPT at its
implied rate $y = \STeff/N$, apply $F$, and the same $\STeff$ returns.
This self-reproducing value is a \emph{fixed point}. \Cref{sec:setup}
formulates it, \cref{sec:existence} proves it exists and is unique, and
\cref{sec:solver} finds it on-chain.

\subsection{Components of a Dusk market}
\label{sec:overview:pieces}

A Dusk market adds three components to the Dawn stack and reuses its
accountant. Each plays one role in resolving the recursion:
...
\begin{itemize}
  \item \textbf{Gyro E-CLP pool:} A Balancer V3 elliptic concentrated
    ...
    share is registered as a \code{WITH_RATE} token: Balancer scales
    its raw balance by the rate provider's \code{getRate()} before any
    pool math, which is how the solved rate enters the pool's pricing
    of senior shares.
  \item \textbf{Dusk kernel:} One contract, two paths. Write: the sync
    ...
    accountant checkpoint, and caches the solved rate $y^*$. Read:
    Balancer's rate provider interface, \code{getRate()}, returns the
    cached $y^*$ instantly.
  \item \textbf{Balancer V3 Dusk hook:} Forwards Balancer's before-operation
    ...
    every operation executes at a fresh senior share rate.
