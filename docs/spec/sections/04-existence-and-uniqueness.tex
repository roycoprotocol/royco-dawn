...
\]
where $V_{\max}$ is the conservative pool value at the upper corner of
the frozen curve, the integer error function $E(x) = F(P_A(x/N)) - x$ is
...
form a prefix $[0, x^\star]$ with a unique right endpoint $x^\star$, the
self-consistent senior NAV, and bisection on the predicate
$E(x) \geq 0$ finds it.
\end{theorem}

The two pillars are proved in \cref{sec:existence:mono,sec:existence:sign}.
...
variable vanishes at the fair point, leaving the direct term. So $V$ is
non-decreasing in $y$.
...
factors,
\[
  \frac{1}{N} \cdot f N \cdot 1 \;=\; f \;\leq\; 1,
\]
...
\[
  E(x{+}1) - E(x) \;\in\; \{-1,\ 0\}.
\]
$E$ is non-increasing, so the integers with $E \geq 0$ form a prefix
$[0, x^\star]$. At its right endpoint the next step must be a drop of
...
fixed point. This proves Monotonicity.
...
\rfloor$, where $V_{\max}$ is the conservative value at the upper corner
of the frozen curve (above the band the pool is all quote and its value
is constant in $y$). By NAV conservation $\STeff = \STraw + \JTraw - \JTeff \leq \STraw
