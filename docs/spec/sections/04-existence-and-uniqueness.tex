...
cancellation holds on the open band $(\alpha, \beta)$, where the fair
point moves smoothly. At the band edges $V$ is pinned at a corner and
flat in $y$, which only helps. So $V$ is non-decreasing in $y$.
...
$x^\star$ is the largest, hence the senior-conservative choice (smallest
$\JTeff$). This proves Monotonicity.
...
\rfloor$, where $V_{\max} = \sup_y V(y)$ is the pool's value at the
all-quote corner (the senior shares all sold). Since $V$ is
