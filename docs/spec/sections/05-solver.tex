...
it. This section runs the solver: integer bisection, in bounded gas, with
the conditions that make the theorem apply on chain stated as explicit
obligations.
...
construction, so the search always brackets a unique answer. Every probe
$x \in \mathcal{B}$ maps to a senior balance within the E-CLP's
computable domain, so no probe reverts inside the pool math.
...
\begin{description}
  \item[Frozen inputs.] Every input to $E$ ($\STraw$, $N$, $f$, the
    ...
    different probes and assumes one fixed function. The loop makes no
    external calls.
...
\begin{description}
  ...
  \item[Frozen curve.] $L$ is computed once per solve from the balances
    ...
    frozen curve.
  \item[Clamp.] Demand is capped at the supply $N$ (read at solve start)
    before the $f V$ floor.
