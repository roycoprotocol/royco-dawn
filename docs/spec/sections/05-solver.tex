...
\begin{description}
  \item[Senior independence.] The senior raw NAV is marked from sources
    that do not read the published rate, so the loop has exactly one
  ...
  \item[Clamp.] The valuation uses the integral form
    ...
    Capping the share count inside the point value, $b_q + y\min(D,N)$,
    is a different function that is not $N$-Lipschitz and breaks
    monotonicity.
