...
$[N/\beta,\,N/\alpha]$ whenever the loop gain $g$ is below $1$ and the
...
the bracket is halved and the half containing the sign change is kept; the
loop runs for a fixed number of steps determined by the bracket width and
the configured tolerance, then returns the midpoint (rounded toward
senior, see \S\ref{sec:solver:rounding}) as $x^*$. The published rate is
$y^* = x^* / N$.
...
The default search bracket is the rate range the pool can express. For an
E-CLP with price bounds $[\alpha, \beta]$, the senior rate is bounded by
the pool geometry, so
\begin{equation}
  x \in \left[\frac{N}{\beta},\, \frac{N}{\alpha}\right].
  \label{eq:default-bracket}
\end{equation}

This is the operating range of the pool, not a universal economic bound.
A senior loss large enough to push the correct rate below $1/\beta$ falls
...
$[N/\beta, N/\alpha]$ is the conservative default. Either way, the
bracket is fixed at the start of each solve from values read once;
...
\begin{description}
  ...
  \item[Iteration count] fixed at
    ...
    \cref{eq:default-bracket}; tolerance from the kernel's storage.
...
the kernel asserts $g_{\text{proxy}} <$ \code{maxLoopGainWad}; if not, the
sync reverts with \code{LOOP_GAIN_EXCEEDED}. Because the proxy
