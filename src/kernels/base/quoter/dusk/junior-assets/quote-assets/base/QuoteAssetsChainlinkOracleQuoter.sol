...
abstract contract QuoteAssetsChainlinkOracleQuoter is RoycoDuskKernel {
     ...
     */
    function __QuoteAssetsChainlinkOracleQuoter_init_unchained(address _quoteAssetOracle, uint48 _quoteAssetStalenessThresholdSeconds)
        internal
        onlyInitializing
    {
    ...
    function lpConvertQuoteAssetsToNAVUnits(QUOTE_UNIT _quoteAssets) public view override(RoycoDuskKernel) returns (NAV_UNIT) {
        return toNAVUnits(
            toUint256(_quoteAssets.mulDiv(_getCachedQuoteAssetToNAVUnitConversionRateWAD(), QUOTE_UNIT_SCALE_FACTOR, Math.Rounding.Floor))
        );
    }
     ...
     */
    function setQuoteAssetChainlinkOracle(address _quoteAssetOracle, uint48 _quoteAssetStalenessThresholdSeconds, bool _syncBeforeUpdate)
        external
        restricted
    {
