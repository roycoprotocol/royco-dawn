 ...
 * @custom:field kernel - The kernel that this accountant maintains accounting for
 * @custom:field coverageWAD - The coverage ratio that the junior tranche is meant to protect senior tranche drawdowns by, scaled to WAD precision
 * @custom:field betaWAD - The JT's sensitivity to the same downside stress that affects ST, scaled to WAD precision
 *                         For example, beta is 0 when JT is in the RFR and 1 when JT is in the same opportunity as senior
 * @custom:field protocolFeeWAD - The market's configured protocol fee percentage taken from yield earned by senior and junior tranches, scaled to WAD precision
 * @custom:field rdm - The market's Reward Distribution Model (RDM), responsible for determining the ST's yield split between ST and JT
 * @custom:field lastSTRawNavRAY - The last recorded raw NAV: the pure value of its invested assets, scaled to RAY precision
 * @custom:field lastJTRawNavRAY - The last recorded raw NAV: the pure value of its invested assets, scaled to RAY precision
 * @custom:field lastSTEffectiveNavRAY - The last recorded effective NAV (including any prior applied coverage, ST yield distributions, and uncovered losses) of the senior tranche, scaled to RAY precision
 * @custom:field lastJTEffectiveNavRAY - The last recorded effective NAV (including any prior provided coverage, JT yield, ST yield distribution, and JT losses) of the junior tranche, scaled to RAY precision
 * @custom:field lastJTCoverageDebtRAY - The losses that ST incurred after exhausting the JT loss-absorption buffer: represents the first claim on capital the senior tranche has on future recoveries, scaled to RAY precision
 * @custom:field lastSTCoverageDebtRAY - The coverage that has been applied to ST from the JT loss-absorption buffer : represents the second claim on capital the junior tranche has on future recoveries, scaled to RAY precision
...
struct RoycoAccountantState {
    ...
    uint96 betaWAD;
    address rdm;
    uint256 lastSTRawNavRAY;
    uint256 lastJTRawNavRAY;
    uint256 lastSTEffectiveNavRAY;
    uint256 lastJTEffectiveNavRAY;
    uint256 lastJTCoverageDebtRAY;
    uint256 lastSTCoverageDebtRAY;
