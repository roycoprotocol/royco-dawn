...
import { IRoycoVaultTranche, RoycoIdenticalAssetTrancheBaseAssetChainlinkOracle } from "./RoycoIdenticalAssetTrancheBaseAssetChainlinkOracle.sol";

/**
 * @title RoycoIdenticalAssetTrancheBaseAssetChainlinkOracleFactory
 ...
 */
contract RoycoIdenticalAssetTrancheBaseAssetChainlinkOracleFactory {
    /// @dev The global deployment salt used for all oracles
    bytes32 private constant ORACLE_DEPLOYMENT_SALT = keccak256(abi.encode("ROYCO_IDENTICAL_ASSET_TRANCHE_BASE_ASSET_CHAINLINK_ORACLE"));
    ...
    /// @notice Constructs the factory for deploying Chainlink compatible Royco tranche base asset share price oracles
    ...
    function deployOracle(address _tranche) external returns (address oracle) {
        ...
        trancheToOracle[_tranche] = oracle = address(new RoycoIdenticalAssetTrancheBaseAssetChainlinkOracle{ salt: ORACLE_DEPLOYMENT_SALT }(_tranche));
    ...
    function predictOracleAddress(address _tranche) external view returns (address) {
        return Create2.computeAddress(
            ORACLE_DEPLOYMENT_SALT, keccak256(abi.encodePacked(type(RoycoIdenticalAssetTrancheBaseAssetChainlinkOracle).creationCode, abi.encode(_tranche)))
        );
    }
