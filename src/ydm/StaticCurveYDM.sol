...
contract StaticCurveYDM is IYDM {
        ...
        returns (uint256)
    {
        ...
        if (utilizationWAD >= WAD) {
            ...
            return SLOPE_GTE_TARGET_UTIL_WAD.mulDiv((utilizationWAD - TARGET_UTILIZATION_WAD), WAD, Math.Rounding.Floor) + BASE_RATE_GTE_TARGET_UTIL_WAD;
        } else {
            // If utilization is below the kink (target), apply the first leg of R(U)
            return SLOPE_LT_TARGET_UTIL_WAD.mulDiv(utilizationWAD, WAD, Math.Rounding.Floor);
        }
    }
}
