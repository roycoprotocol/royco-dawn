...
contract AccountantAttributionFuzzTest is BaseTest {
    ...
    ///      `testFuzz_fixHolds_stEffectiveNAVStaysFlatWhenStSupplyIsZero`.
    ...
    function _setUpRoyco() internal override {
        super._setUpRoyco();
        MarketDeployment memory result = _deployMarket(0.5e18);
    ...
    function _deployMarket(uint256 _jtYieldShareWAD) internal returns (MarketDeployment memory) {
        MarketDeploymentConfig.IdenticalERC4626SharesToAdminOracleQuoterKernelParams memory kernelParams =
            MarketDeploymentConfig.IdenticalERC4626SharesToAdminOracleQuoterKernelParams({ initialConversionRateWAD: WAD });

        MarketDeploymentConfig.AdaptiveCurveYDM_V2_Params memory ydmParams = MarketDeploymentConfig.AdaptiveCurveYDM_V2_Params({
        ...
        MarketDeploymentConfig.MarketConfig memory config = MarketDeploymentConfig.MarketConfig({
            ...
            stDustTolerance: 1,
            jtDustTolerance: 1,
            component: COMPONENT_ID_KERNEL_IDENTICAL_ERC4626_ADMIN_ORACLE,
            ...
            ydmType: MarketDeploymentConfig.YDMType.AdaptiveCurve_V2,
            transferAgentAddress: address(0),
            ydmSpecificParams: abi.encode(ydmParams)
        });

        DeployScript.DeploymentResult memory r =
            DEPLOY_SCRIPT.deploy(config, OWNER_ADDRESS, PROTOCOL_FEE_RECIPIENT_ADDRESS, _roleAssignmentAddresses(), DEPLOYER.privateKey);
        AM = r.accessManager;
        FACTORY = r.factory;
