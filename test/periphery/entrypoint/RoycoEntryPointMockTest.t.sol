...
contract RoycoEntryPointMockTest is Test {
    ...
    function setUp() public {
        ...
        // Setup LP role admin on the AccessManager (the factory's `authority()`).
        vm.startPrank(owner);
        am.grantRole(LP_ROLE_ADMIN_ROLE, lpRoleAdmin, 0);
        ...
        // Grant LP roles on the AM via the LP_ROLE_ADMIN_ROLE holder.
        vm.startPrank(lpRoleAdmin);
        am.grantRole(ST_LP_ROLE, userA, 0);
        am.grantRole(JT_LP_ROLE, userA, 0);
        am.grantRole(ST_LP_ROLE, userB, 0);
        am.grantRole(JT_LP_ROLE, userB, 0);
        am.grantRole(ST_LP_ROLE, executor, 0);
        am.grantRole(JT_LP_ROLE, executor, 0);
        am.grantRole(ST_LP_ROLE, address(entryPoint), 0);
        am.grantRole(JT_LP_ROLE, address(entryPoint), 0);
        vm.stopPrank();
    ...
    function _mockEntryPointAccess() internal {
        // Mock canCall on the AM (the factory's `authority()`) to always return (true, 0). The
        // entry point's `restricted` modifier routes through `_authority()` → AM.canCall(...).
        vm.mockCall(address(am), abi.encodeWithSignature("canCall(address,address,bytes4)"), abi.encode(true, uint32(0)));
    }

    /// @dev Stands up a fresh AM + factory proxy via the new template-driven factory's 1-arg
    ///      `initialize(am)` flow. Mirrors `BaseTest._bootstrapFactory()` / `DEPLOY_SCRIPT.bootstrapFactory(...)`.
    function _deployFactory() internal {
        am = new AccessManager(owner);

        RoycoFactory factoryImpl = new RoycoFactory();

        // The factory's `initialize` requires the factory itself to hold ADMIN_ROLE on the AM
        // BEFORE the initializer fires, so we pre-grant the role to the predicted proxy address.
    ...
    function test_requestDeposit_revert_trancheNotEnabled() public {
        ...
        am.grantRole(ST_LP_ROLE, address(disabledEntryPoint), 0);
    ...
    function test_modifyTrancheConfigs_revert_notAdmin() public {
        ...
        RoycoEntryPoint newEntryPointImpl = new RoycoEntryPoint(address(factory));
        ...
        bytes memory initData = abi.encodeCall(RoycoEntryPoint.initialize, (initTranches, initConfigs));
    ...
    function test_initialize_lengthMismatch_reverts() public {
        RoycoEntryPoint newImpl = new RoycoEntryPoint(address(factory));
