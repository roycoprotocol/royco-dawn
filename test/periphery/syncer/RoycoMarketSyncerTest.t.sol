...
contract RoycoMarketSyncerTest is Test {
    ...
    /// @dev Stands up the AM + factory via the new template-driven factory's 1-arg
    ///      `initialize(am)` flow. AM holds all the role bindings the syncer relies on.
    function _deployFactory() internal {
        am = new AccessManager(FACTORY_ADMIN_ADDRESS);
        RoycoFactory factoryImpl = new RoycoFactory();

        // The factory's `initialize` asserts the factory holds ADMIN_ROLE on the AM before the
        // initializer fires, so we pre-grant the role to the predicted proxy address.
        ...
        bytes memory initData = abi.encodeCall(RoycoFactory.initialize, (address(am)));
    ...
    function _configureProductionRoles() internal {
        ...
        DeploySyncerScript.SafeTransaction[] memory transactions = deployScript.buildSyncerConfigTransactions(address(am), address(syncer), syncOperators);
        ...
        vm.prank(FACTORY_ADMIN_ADDRESS);
        am.grantRole(ADMIN_UNPAUSER_ROLE, PAUSER_ADDRESS, 0);

        // Grant ADMIN_UPGRADER_ROLE to DEPLOYER (not included in buildSyncerConfigTransactions)
        vm.prank(FACTORY_ADMIN_ADDRESS);
        am.grantRole(ADMIN_UPGRADER_ROLE, DEPLOYER_ADDRESS, 0);
    }
    ...
    /// @dev Post-migration the factory's `ROYCO_AUTHORITY()` is the standalone AccessManager.
    ///      `RoycoMarketSyncer.initialize` wires its `authority()` to that AM, not the factory.
    function test_initialize_setsCorrectAuthority() external view {
        assertEq(syncer.authority(), address(am), "Authority should be the AccessManager");
    }
    ...
    function test_upgrade_authorizedCanUpgrade() external {
        ...
        RoycoMarketSyncer newImpl = new RoycoMarketSyncer(address(factory));
        ...
        assertEq(syncer.authority(), address(am), "Authority should remain after upgrade");
    }
    ...
    function test_productionConfig_transactionCount_noOperators() external view {
        DeploySyncerScript.SafeTransaction[] memory txs = deployScript.buildSyncerConfigTransactions(address(am), address(syncer), new address[](0));
        assertEq(
    ...
    function test_productionConfig_transactionCount_withOperators() external view {
        ...
        DeploySyncerScript.SafeTransaction[] memory txs = deployScript.buildSyncerConfigTransactions(address(am), address(syncer), syncOperators);
    ...
    function test_productionConfig_allTransactionsTargetFactory() external view {
        ...
        for (uint256 i = 0; i < txs.length; i++) {
            // Post-migration the AM is a separate contract from the factory; role txs target the AM.
            assertEq(txs[i].to, address(am), string.concat("Transaction ", vm.toString(i), " should target the AccessManager"));
    ...
    function test_productionConfig_syncRoleFunctions() external view {
        ...
        for (uint256 i = 0; i < syncSelectors.length; i++) {
            (bool canCall, uint32 delay) = am.canCall(SYNC_OPERATOR_ADDRESS, address(syncer), syncSelectors[i]);
    ...
    function test_productionConfig_pauserRoleFunctions() external view {
        (bool canCall, uint32 delay) = am.canCall(PAUSER_ADDRESS, address(syncer), IRoycoAuth.pause.selector);
    ...
    function test_productionConfig_unpauserRoleFunctions() external view {
        (bool canCall, uint32 delay) = am.canCall(PAUSER_ADDRESS, address(syncer), IRoycoAuth.unpause.selector);
    ...
    function test_productionConfig_upgraderRoleFunction() external view {
        (bool canCall,) = am.canCall(DEPLOYER_ADDRESS, address(syncer), syncer.upgradeToAndCall.selector);
    ...
    function test_productionConfig_unauthorizedCannotCallAnyFunction() external view {
        ...
        for (uint256 i = 0; i < allRestrictedSelectors.length; i++) {
            (bool canCall,) = am.canCall(UNAUTHORIZED_USER_ADDRESS, address(syncer), allRestrictedSelectors[i]);
    ...
    function test_productionConfig_roleSeparation_syncCannotPause() external view {
        (bool canCall,) = am.canCall(SYNC_OPERATOR_ADDRESS, address(syncer), IRoycoAuth.pause.selector);
        assertFalse(canCall, "SYNC_ROLE should NOT be able to pause");

        (canCall,) = am.canCall(SYNC_OPERATOR_ADDRESS, address(syncer), IRoycoAuth.unpause.selector);
    ...
    function test_productionConfig_roleSeparation_pauserCannotSync() external view {
        (bool canCall,) = am.canCall(PAUSER_ADDRESS, address(syncer), RoycoMarketSyncer.executeBatchAccountingSync.selector);
        assertFalse(canCall, "ADMIN_PAUSER_ROLE should NOT be able to sync");

        (canCall,) = am.canCall(PAUSER_ADDRESS, address(syncer), RoycoMarketSyncer.addMarketKernels.selector);
    ...
    function test_productionConfig_roleSeparation_syncCannotUpgrade() external view {
        (bool canCall,) = am.canCall(SYNC_OPERATOR_ADDRESS, address(syncer), syncer.upgradeToAndCall.selector);
    ...
    function test_productionConfig_usesCorrectRoleIds() external view {
        ...
        uint64 syncRole = am.getTargetFunctionRole(address(syncer), RoycoMarketSyncer.executeBatchAccountingSync.selector);
        uint64 addKernelsRole = am.getTargetFunctionRole(address(syncer), RoycoMarketSyncer.addMarketKernels.selector);
        uint64 removeKernelsRole = am.getTargetFunctionRole(address(syncer), RoycoMarketSyncer.removeMarketKernels.selector);
        uint64 syncForRole = am.getTargetFunctionRole(address(syncer), RoycoMarketSyncer.executeBatchAccountingSyncFor.selector);
        uint64 pauseRole = am.getTargetFunctionRole(address(syncer), IRoycoAuth.pause.selector);
        uint64 unpauseRole = am.getTargetFunctionRole(address(syncer), IRoycoAuth.unpause.selector);
        uint64 upgradeRole = am.getTargetFunctionRole(address(syncer), syncer.upgradeToAndCall.selector);
